			...
			<div style="height: 86px;margin-bottom:40px;background: #fff;">
				<div style="width: 1226px;margin:0 auto;">
					...
					<div style="float: right;">
                        ...
                        <div class="click" style="float: right;width:100px;text-align: center;margin-right:15px;color:#ac1f1f;font-size:18px;height:24px;margin-top:31px; "  v-on:click="addDealerOpt()" v-show="adminshow">新建子账号</div>
					</div>
				</div>
			</div>
...
<!--									<div class="paymentType_2" v-show="selectDealerPaymentType==2">自收费</div>-->
									<div class="editDealer" v-on:click="editDealerAccountBtn()">编辑</div>
									<div class="deleteDealer" v-on:click="deleteCommit(3, 1, 0)"  v-show="adminshow&&selectedDealerIsAdmin!=1">删除</div>
									
								</div>
								<div style="margin-left: 50px;" class="ticketCount" v-show="selectDealerID!=-1">
									房卡库存：{{inventoryCount}}张  <span class="rechargeBtn" v-on:click="openRechargePart()" v-show="adminshow">充值</span>
							...
							<div class="roomCard mainPart" v-show="part==1">							
                                ...
                                <div class="roomCardSelectList">
                                    <div class="selectItem"  v-on:click="roomCardPartSelect(1)"v-show="roomCardPart!=1">管理员房卡日志</div>
                                    <div class="selectItem selected"  v-show="roomCardPart==1">管理员房卡日志</div>
                                    <div class="selectItem"  v-on:click="roomCardPartSelect(2)"v-show="roomCardPart!=2">用户充值日志</div>
                                    <div class="selectItem selected"  v-show="roomCardPart==2">用户充值日志</div>
                                </div>

								<table class="saleList" border="0" cellspacing="0" cellpadding="0" align="center">				
									<tr class="saleListTh" >
										<td style="width: 20%">时间</td>
										<td style="width: 30%">用户<span v-show="roomCardPart==2">及ID</span></td>
                                        					<td style="width: 15%">类型</td>
										<td style="width: 20%">操作</td>
										<td style="width: 15%">房卡</td>
									</tr>
									<tr class="saleListTr" v-for="j in journalList" v-show="roomCardPart==1">
										<td >{{j.time}}</td>
										<td >{{selectDealerName}}</td>
                                        					<td >{{j.journal_type}}</td>
										<td >{{j.content}}</td>
										<td >{{j.ticket_count}}张</td>
									</tr>
				                                    <tr class="saleListTr" v-for="j in ManualRechargeList" v-show="roomCardPart==2">
				                                        <td >{{j.time}}</td>
				                                        <td >{{j.user}}[ID:{{j.uid}}]</td>
				                                        <td >{{j.journal_type}}</td>
				                                        <td >{{j.content}}</td>
				                                        <td >{{j.ticket_count}}张</td>
				                                    </tr>
								...
								<div style="width:100%;" v-show="totalPage>1">
								   <div class="tcdPageCode tcdPageCode1" v-show="roomCardPart==1"></div>
								   <div class="tcdPageCode tcdPageCode10" v-show="roomCardPart==2"></div>
								</div>	
							...
							<div class="mainPart user" v-show="part==3">
								...
								<table class="result" cellspacing="0">
									<tr class="title">
										...
										<td style="width: 20%;">玩家名称</td>
										<td style="width: 24%;">积分</td>
										<td style="width: 18%;">房卡</td>
                                        					<td style="width: 30%;">操作</td>
									</tr>
									<tr class="item" v-for="(u,index) in userSearch.data" >
										<td>{{u.user_code}}</td>
										<td>{{u.nickname}}</td>
										<td>{{u.sum_score}}</td>
										<td>{{u.ticket_count}}</td>
					                                        <td>
					                                            <div>
					                                                <span  style="width: 50%; color: #397abe;" v-on:click="showDetail(u.account_id)"> 查看积分</span>
					                                                <span  style="width: 50%; color:#397abe;" v-on:click="openUserRechargePart(u.nickname, u.user_code, u.account_id)">充值</span>
					                                            </div>
					                                        </td>
									</tr>
								</table>
...
<!--										<option v-for="gr in gamescore_array" v-bind:value="gr.game_type" v-if="gr.is_show==1">{{gr.game_title}}</option>-->
							...
							<div class="mainPart roomCard"  v-show="part==9">
								<div class="search">
									...
									<div class="button" style="background:#397abe; color:white;"  v-on:click="searchCardInfo()">查询</div>	
								</div>
								<div v-show="!roomCard.turn">
									<table class="roomCardList" cellspacing="0" >
										...
										<tr class="item" v-for="c in roomCard.data" >
											...
											<td ><div class="cardCancel" v-on:click="cardCancel(c.account_id,c.ticket_count)" v-show="adminshow">注销房卡</div></td>
							...
							<div class="mainPart agent"  v-show="part==11">
								<div class="search">
									...
									<div class="button" style="background:#397abe; color:white;" v-on:click="searchAgent()">查询</div>	
								</div>
...
<!--                                            <div class="button" style="display: inline-block">刷新</div>-->
		...
		<div class="editDealerPart" v-show="editDealerPartShow">
			...
			<div class="infoPart">
				...
				<div class="infoItem" >密&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp码：<input v-model="editDealerPwd" type="password" /></div>
                <div class="infoItem" >手&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp机：<input v-model="editDealerPhone" /></div>
				<!--<div class="infoItem" >权&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp限：
					<select id="permissionType" v-model="editDealerPermission" >
						<option class="selectedPermission" value="-1">请选择</option>
						<option class="selectedPermission" value="1">允许充值</option>
						<option class="selectedPermission" value="2">不允许充值</option>
					</select>
				</div>-->
			</div>
		...
		<div class="personDetail" v-show="userSearch.isShowDetail" v-on:click="hideDetail()">
			<div class="mainPart" style="width: 610px;left: 36%; top: 33%" v-on:click.stop="hideDetail(true)">
                <div style="float: left;">
                    日期选择:
                </div>
                <div class="control-group" style="float: left;margin-left: 5px;">
        ...
        <!-- 编辑用户充值信息 -->
            <div class="userRechargeCard" v-show="userRechargeShow">
                <div class="titleText">用户房卡充值</div>
                <div class="infoPart">用户名： {{userRechargeName}}</div>
                <div class="infoPart">用户id： {{userRechargeId}}</div>
                <div class="infoPart">
                    <div class="infoItem" >房&nbsp&nbsp卡&nbsp&nbsp数：<input v-model="userRechargeCount" onkeyup="this.value=this.value.replace(/[^0-9]/g,'')" onafterpaste="this.value=this.value.replace(/[^0-9]/g,'')"/></div>
                </div>
                <div class="buttonPart" >
                    <div class="left click" v-on:click="saveUserRechargeOpt()">确定</div>
                    <div class="right click" v-on:click="closeUserRechargePart()">取消</div>
                </div>
            </div>
