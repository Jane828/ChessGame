...
class Account_Model extends Admin_Common_Model {
    ...
    */
    public function addDealerOpt($arrData) {
        ...
        if (!isset($arrData['phone']) || $arrData['phone'] === "") {
            log_message('error', "function(addDealerOpt):lack of phone" . " in file" . __FILE__ . " on Line " . __LINE__);
            return $this->missingPrameterArr("phone");
        }

        if (strlen($arrData['passwd']) < 6 || strlen($arrData['passwd'])>18){
            return array('result' => OPT_CONST::FAILED, 'data' => $result, 'result_message' => "请输入长度6-18的密码");
        }

        if (!is_numeric($arrData['phone'])){
            return array('result' => OPT_CONST::FAILED, 'data' => $result, 'result_message' => "手机号非法");
        }
        ...
        if ($dealer_id == -1) {
            ...
            $dealer_sql   = 'select dealer_num,name,account,passwd from ' . D_Dealer_Account . ' where ' . $dealer_where;
            ...
            $account_array['phone']        = $phone;
            $account_array['dealer_num']   = "2";
            ...
            $account_array['update_time']  = $timestamp;
            $account_array['update_appid'] = "admin_" . $_SESSION["LoginAdminID"];
            $account_array['is_delete']    = 0;
            $account_array['name']         = $name;
            if ($dealer_id != $_SESSION["LoginAdminID"]) {
                $account_array['account']      = $account;
            }
            $account_array['clear_pwd']    = $clear_pwd;
            $account_array['passwd']       = $passwd;
    ...
    */
    public function delDealerOpt($arrData) {
        ...
        $updateTicket_where = 'dealer_id="' . $dealer_id . '" and account !="' . G_CONST::SUPER_ADMIN . '"';
    ...
    */
    public function getDealerList($arrData) {
        ...
        $other_contion = "";
        if ($_SESSION['LoginUser'] != G_CONST::SUPER_ADMIN) {
            $dealer_where .= " and dealer_id = " . $_SESSION['LoginAdminID'];
        } else {
            $other_contion = " order by create_time desc";
        }
        ...
        $dealer_sql   = 'select dealer_id,dealer_num,name,account,phone, account="' . G_CONST::SUPER_ADMIN . '" as is_admin from ' . D_Dealer_Account . ' where ' . $dealer_where . $other_contion;
    ...
    */
    public function dealerRechargeOpt($arrData) {
        ...
        if (!isset($arrData['dealer_id']) || $arrData['dealer_id'] === "") {
            log_message('error', "function(dealerRechargeOpt):lack of dealer_id" . " in file" . __FILE__ . " on Line " . __LINE__);
            return $this->missingPrameterArr("dealer_id");
        }
