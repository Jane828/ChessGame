    ...
    */
    ...
    public function loginOpt() {
        if (isset($_POST['account']) && isset($_POST['pwd'])) {
            ...
            $arrData['auth_code'] = $_POST['auth_code'];
    ...
    public function getMobileSmsForLogin() {
        if (isset($_POST['account']) && isset($_POST['phone'])) {
            $arrData['account'] = $_POST['account'];
            $arrData['phone']   = $_POST['phone'];

            $this->load->model('account/account_model', '', TRUE);
            $result = $this->account_model->getMobileSmsForLogin($arrData);
            print_r(json_encode($result));
        }
    }
    ...
    public function index() {
        ...
        $data['base_url']   = base_url();
        $data['superAdmin'] = $_SESSION["LoginUser"] == G_CONST::SUPER_ADMIN ? 1 : 0;
    ...
    public function updateDealerOpt() {
        ...
        if (!isset($params_ary['dealer_id']) || $params_ary['dealer_id'] == -1) {
            $this->checkAdmin();
        }
        ...
        $request_array['phone']     = $params_ary['phone'];
     ...
     * 代理商手动充值记录
     */
    public function dealerManualRechargeJournal() {
        $this->checkOptLogin();
        ...
        $request_array['page'] = $params_ary['page'];
        $this->load->model('admin/account_model', 'admin_account_model', TRUE);
        $result = $this->admin_account_model->getDealerManualRechargeJournal($request_array);
        $result = json_encode($result);
    ...
    public function roomCard() {
        ...
        $data['adminshow'] = $_SESSION["LoginUser"] == G_CONST::SUPER_ADMIN ? 1 :0;
