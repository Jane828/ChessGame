...
var httpModule = {
    ...
    getScoreInfo: function () {
        Vue.http.post(globalData.baseUrl + 'f/scoreStat', {'type':71}).then(function (response) {
            var bodyData = response.body;
...
var socketModule = {
    ...
    processGameRule: function(obj) {
        ...
        if (appData.ruleInfo.isCardfive == 1
            && appData.ruleInfo.isCardbomb == 1
            && appData.ruleInfo.isCardtiny === 1
            && appData.ruleInfo.isStraight === 1
            && appData.ruleInfo.isFlush === 1
            && appData.ruleInfo.isHulu === 1) {

            appData.ruleInfo.rule_height = '11vh';

        } else {
            if (appData.ruleInfo.isCardfive == 1 || appData.ruleInfo.isCardbomb == 1
                || appData.ruleInfo.isCardtiny === 1
                || appData.ruleInfo.isStraight === 1
                || appData.ruleInfo.isFlush === 1
                || appData.ruleInfo.isHulu === 1) {
                ...
                appData.ruleInfo.rule_height = 0;
            }
        }
    ...
    processUpdateAccountStatus: function(obj) {
        ...
        for (var i = 0; i < 10; i++) {
            if (appData.player[i].account_id == obj.data.account_id) {
                ...
                if (appData.player[i].account_status >= 8 && obj.data.account_status != 2) {
...
var viewMethods = {
    ...
    resetCardOver: function(num) {
        if (num == 1) {
            ...
            $(".cardOver .card651").css("left", "40%");
            $(".cardOver .card641").css("left", "44%");
            $(".cardOver .card631").css("left", "48%");
            $(".cardOver .card621").css("left", "52%");
            $(".cardOver .card611").css("left", "56%");
        } else if (num == 7) {
            $(".cardOver .card711").css("left", "17vh");
            $(".cardOver .card721").css("left", "15vh");
            $(".cardOver .card731").css("left", "13vh");
            $(".cardOver .card741").css("left", "11vh");
            $(".cardOver .card751").css("left", "9vh");
        }else if (num == 8) {
            $(".cardOver .card811").css("left", "17vh");
            $(".cardOver .card821").css("left", "15vh");
            $(".cardOver .card831").css("left", "13vh");
            $(".cardOver .card841").css("left", "11vh");
            $(".cardOver .card851").css("left", "9vh");
        }
        else if (num == 9) {
    ...
    updateAllPlayerStatus: function() {
        for (var i = 0; i < appData.player.length; i++) {
            ...
            if (appData.player[i].card_type >= 1) {
                ...
                if (cardType == 6) {
                    appData.player[i].is_bull = true;
                    appData.player[i].bullImg = globalData.imageUrl + "files/images/lbull/straight.png";
                } else if (cardType == 7) {
                    appData.player[i].is_bull = true;
                    appData.player[i].bullImg = globalData.imageUrl + "files/images/lbull/flush.png";
                } else if (cardType == 8) {
                    appData.player[i].is_bull = true;
                    appData.player[i].bullImg = globalData.imageUrl + "files/images/lbull/hulu.png";
                }
            }
            if (appData.player[i].account_status == 4) {
...
var audioModule = {
    ...
    loadAllAudioFile: function() {
        ...
        var audioUrl = ["nobanker.m4a", "robbanker.m4a", "nobull.m4a", "bull1.m4a", "bull2.m4a", "bull3.m4a", "bull4.m4a", "bull5.m4a", "bull6.m4a", "bull7.m4a", "bull8.m4a", "bull9.m4a", "bull10.m4a", "bullflower.m4a", "bullbomb.m4a", "bulltiny.m4a", "times1.m4a", "times2.m4a", "times3.m4a", "times4.m4a", "times8.m4a", "times5.m4a", "times10.m4a", "coin.mp3", "audio1.m4a"];
        var audioName = ["audioNoBanker", "audioRobBanker", "audioBull0", "audioBull1", "audioBull2", "audioBull3", "audioBull4", "audioBull5", "audioBull6", "audioBull7", "audioBull8", "audioBull9", "audioBull10", "audioBull11", "audioBull12", "audioBull13", "audioTimes1", "audioTimes2", "audioTimes3", "audioTimes4", "audioTimes8", "audioTimes5", "audioTimes10", "audioCoin", "audio1"];
        ...
        var audioLaizi = ['flush.mp3', 'hulu.mp3', 'straight.mp3'];
        var audioLaiziName = ['flush', 'hulu', 'straight'];
        for (var i = 0; i < audioLaizi.length; i++) {
            this.loadAudioFile(this.baseUrl + 'files/audio/lbull/' + audioLaizi[i], audioLaiziName[i]);
        }
    }
};
...
function getShareContent() {
    ...
    if (appData.ruleInfo.banker_mode == 1) {
        ...
        shareContent = shareContent + '模式：明牌抢庄 ';
    } else if (appData.ruleInfo.banker_mode == 3) {
        shareContent = shareContent + '模式：牛牛上庄 ';
    } else if (appData.ruleInfo.banker_mode == 4) {
        shareContent = shareContent + '模式：通比牛牛 ';
    } else if (appData.ruleInfo.banker_mode == 5) {
    ...
    if (appData.ruleInfo.baseScore == 1) {
        ...
        shareContent = shareContent + '底分：20分';
    ...
    if (appData.ruleInfo.timesType == 1) {
        ...
        shareContent = shareContent + '  规则：牛牛x4牛九x3牛八x2牛七x2';
    }
    ...
    if (appData.ruleInfo.isCardfour == 1 || appData.ruleInfo.isCardfive == 1 || appData.ruleInfo.isCardbomb == 1 || appData.ruleInfo.isCardtiny == 1) {
        ...
        if (appData.ruleInfo.isStraight == 1) {
            cardContent = cardContent + ' 顺子牛(6倍)';
        }
        ...
        if (appData.ruleInfo.isHulu == 1) {
            cardContent = cardContent + ' 葫芦牛(6倍)';
        }

        if (appData.ruleInfo.isCardbomb == 1) {
            cardContent = cardContent + ' 炸弹牛(6倍)';
        }
