...
var httpModule = {
    loadMoreMemberList: function () {
        Vue.http.get(globalData.apiUrl + 'club/applies?page=' + appData.page + '&club_no=' + globalData.club_no).then(function(response) {
            var bodyData = response.body;
            appData.isHttpRequest = false;
            if (bodyData.code == 0) {
                appData.sumPage = bodyData.totalPage;
                for (var i = 0; i < bodyData.data.length;i++) {
                    var item = bodyData.data[i];
                    appData.gameScoreList.push({"nick":item.nick,"ucode":item.ucode,"status":item.status,"avatar":item.head});
                }
            } else {
            ...
            console.log(appData.canLoadMore);
            if (appData.gameScoreList.length >= 1) {
                appData.noInfo = 0;
            } else {
                appData.noInfo = 1;
            }
    ...
    dealApply: function (action, ucode) {
        var data = {
        ...
        console.log(data);
        Vue.http.post(globalData.apiUrl + 'club/dealJoin', data).then(function(response) {
            var bodyData = response.body;
            if (bodyData.code == 0) {
                appData.selectedItem.status = bodyData.status;
...
var methods = {
    ...
    clickAgree: function (item) {
        ...
        viewMethods.clickShowAlert(41,"是否同意【" + item.nick + '】加入公会？');
    },
    clickDisagree: function (item) {
        appData.selectedItem = item;
        viewMethods.clickShowAlert(42,"是否拒绝【" + item.nick + '】加入公会？');
    },
    clickRemove: function (item) {
        appData.selectedItem = item;
        viewMethods.clickShowAlert(43,"是否踢【" + item.nick + '】出群？');
    },
    confirmAgree: function () {
        httpModule.dealApply('agree',appData.selectedItem.ucode);
    },
    confirmDisagree: function () {
        httpModule.dealApply('refuse',appData.selectedItem.ucode);
    },
};
