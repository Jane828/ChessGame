...
class Box_Model extends Public_Models
{
    ...
    public function addBox($arrData){
        ...
        $box_number = $account_id * 100000 + rand(1,10000);
    ...
    public function getBoxList($arrData){
        ...
        $result = array();
        $account_id = intval($arrData["account_id"]);
        $query_sql = "select A.box_id, A.account_id, A.box_number, A.game_type, A.status, A.room_count, A.box_name, B.nickname, config from ". Box_Info . ' A, '.WX_Account." B where A.account_id = $account_id and A.account_id=B.account_id";
        $box_list = $this->getDataBySql($dealerDB, 0, $query_sql);
        ...
        $box_array = array();
        foreach($box_list as $key=>$value){
            $box_array[] = $value['box_id'];
        }
        $box_list_str = "'".implode("','",$box_array)."'";
        $time = date('Y-m-d H:i:s', time() - 1200);
        $query_sql = ' select count(id) as num, box_id from '. Box_Room ." where create_time<'$time' and box_id in($box_list_str) group by box_id";
        $room_count = $this->getDataBySql(Game_CONST::DBConst_Name, 0, $query_sql);
        if(is_array($room_count)){
            foreach($box_list as $key=>$value){
                foreach($room_count as $k=>$v){
                    if($value['box_id'] == $v['box_id']){
                        $box_list[$key]['player'] = $v['num']*$this->getRoomMenber($value['game_type']);
                    }
                }
            }
        }
    ...
    public function getBoxInfo($arrData){
        ...
        return array('result'=>OPT_CONST::SUCCESS,'data'=>$box_info,'result_message'=>"查找包厢信息成功");
    }
    ...
    public function setBoxInfo($arrData){
        ...
        $box_config = $this->checkBoxConfig($box_config);
        if($box_config == OPT_CONST::FAILED){
    ...
    public function setBoxStatus($arrData){
        if(!isset($arrData) || empty($arrData['account_id']) || empty($arrData['box_id']) || !isset($arrData['status'])){
            log_message('error', 'function(getBoxInfo): parameter error arrData:'. json_encode($arrData) .' in file '.__FILE__.' on Line '.__LINE__);
            return $this->missingPrameterArr("parameter");
        }
        ...
        return array('result'=>OPT_CONST::SUCCESS,'data'=>array(),'result_message'=>'更新包厢状态成功');
    }
    ...
    public function boxCondition($arrData){
        ...
        $box_info = $this->getDataBySql(Game_CONST::DBConst_Name, 0, $query_sql);
        if($box_info == DB_CONST::DATA_NONEXISTENT){
            return array('result'=>OPT_CONST::FAILED,'data'=>array(),'result_message'=>'未找到包厢信息');
        }
        $game_type = $box_info[0]['game_type'];
        $table = $this->getGameTable($game_type);

        $query_sql = ' select room_id from '.Box_Room.' where box_id ='.$box_id." and create_time > '$time' order by room_id desc";
        ...
        $query_sql = " select A.*, B.game_num,B.player_num from (select room_id, create_time,room_number,account_id,is_close,room_status,game_type,times_type from $table where room_id in($room_list_str)) as A left join in ".Room_Scoreboard." B on A.room_id = B.room_id";
        ...
        $room_list = $this->getRoomExec($room_list, $game_type);
        foreach($box_condition as $key=>$value){
            $value['create_time'] = date('Y-m-d H:i:s', $value['create_time']);
            if(isset($room_list[$value['room_id']])){
                $room_info = $room_list[$value['room_id']];
    ...
    public function checkBoxConfig($config, $box_number){
        ...
        $mould_bu = array("data_key","ticket_type","score_type","rule_type","is_cardfour","is_cardfive",
            "is_cardbomb","is_cardtiny","is_straight","is_flush","is_hulu","is_straightflush","has_ghost",
            "banker_mode","banker_score_type","times_type","score_value","countDown", "game_type");

        $mould_fl = array("data_key","chip_type","ticket_count","disable_pk_men","upper_limit","game_type",
            "raceCard","seenProgress","compareProgress","extraRewards","default_score","countDown");

        $mould_sg = array("data_key","banker_mode","score_type","is_joker","is_bj","ticket_type","countDown");
        $mould = array();
        switch ($game_type){
            case '5':
            case '9':
    ...
    public function getRoomExec($room_list, $game_type){
        ...
        foreach($room_list as $room_id=>$value){
            ...
            $player_num           = $redis->zCard($prefix . "RoomSeq:" . $room_id);
            $room_list[$room_id] = array(
                'total_num'      => $redis_data['totalnum'],
                'game_num'       => $redis_data['gnum'],
