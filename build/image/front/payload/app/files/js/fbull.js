...
var httpModule = {
    getActivityInfo: function() {
        var postData = {"account_id": userData.accountId, "dealer_num": globalData.dealerNum, "session": globalData.session,'room_number' : globalData.roomNumber ,'game_type' : 13 };
    ...
    getScoreInfo: function () {
        Vue.http.post(globalData.baseUrl + 'f/scoreStat', {'type':13}).then(function (response) {
            var bodyData = response.body;
...
var socketModule = {
    ...
    processGameRule: function(obj) {
        ...
        if (appData.ruleInfo.banker_mode == 1) {
            ...
            appData.ruleInfo.bankerText = '抢庄';
        } else if (appData.ruleInfo.banker_mode == 3 && appData.game.round == 1) {
    ...
    processJoinRoom: function(obj) {
        ...
        for (var i = 0; i < 13; i++) {
            if (i <= 13 - obj.data.serial_num) {
                appData.player[i].serial_num = i + Math.ceil(obj.data.serial_num);
            } else {
                appData.player[i].serial_num = i + Math.ceil(obj.data.serial_num) - 13;
            }
        }
    ...
    processAudience: function(obj) {
        ...
        for (var i = 0; i < 13; i++) {
            if (i <= 13 - obj.data.seat_num) {
                appData.player[i].serial_num = i + Math.ceil(obj.data.seat_num);
            } else {
                appData.player[i].serial_num = i + Math.ceil(obj.data.seat_num) - 13;
            }
        }
    ...
    processGameStart: function(obj) {
        ...
        if (appData.game.round == 1 && appData.ruleInfo.banker_mode == 5) {
            ...
            viewMethods.resetMyAccountStatus();
            appData.showClockRobText = true;
        } else if(appData.ruleInfo.banker_mode == 4 || appData.ruleInfo.banker_mode == 3 || appData.ruleInfo.banker_mode == 5) {
            appData.game.time = -1;
            viewMethods.reDeal();
        } else {
            appData.game.time = Math.ceil(obj.limit_time);
            ...
            obj.data.map((round) => {
                round.players.map((player) => {
                    ...
                    cardArr.map( (value) => {
                        ...
                        let y = -storeMethods.setPosition(col)*30;
                        let x = -(row-1)*25;
                        cards.push({
                            x:x,
                            y:y
...
var viewMethods = {
    ...
    resetCardOver: function(num) {
        if (num == 1) {
            ...
            $(".cardOver .card711").css("right", "18vh");
            $(".cardOver .card721").css("right", "20vh");
            $(".cardOver .card731").css("right", "22vh");
            $(".cardOver .card741").css("right", "24vh");
            $(".cardOver .card751").css("right", "26vh");
        } else if (num == 8) {
            $(".cardOver .card811").css("left", "18vh");
            $(".cardOver .card821").css("left", "20vh");
            $(".cardOver .card831").css("left", "22vh");
            $(".cardOver .card841").css("left", "24vh");
            $(".cardOver .card851").css("left", "26vh");
        } else if (num == 9) {
            $(".cardOver .card911").css("left", "40%");
            $(".cardOver .card921").css("left", "44%");
            $(".cardOver .card931").css("left", "48%");
            $(".cardOver .card941").css("left", "52%");
            $(".cardOver .card951").css("left", "56%");
        } else if (num == 10) {
            $(".cardOver .card1011").css("left", "24%");
            $(".cardOver .card1021").css("left", "28%");
            $(".cardOver .card1031").css("left", "32%");
            $(".cardOver .card1041").css("left", "36%");
            $(".cardOver .card1051").css("left", "40%");
        }else if (num == 11) {
            $(".cardOver .card1111").css("left", "10.5vh");
            $(".cardOver .card1121").css("left", "12.5vh");
            $(".cardOver .card1131").css("left", "14.5vh");
            $(".cardOver .card1141").css("left", "16.5vh");
            $(".cardOver .card1151").css("left", "18.5vh");
        } else if (num == 12) {
            ...
            $(".cardOver .card1311").css("left", "17vh");
            $(".cardOver .card1321").css("left", "15vh");
            $(".cardOver .card1331").css("left", "13vh");
            $(".cardOver .card1341").css("left", "11vh");
            $(".cardOver .card1351").css("left", "9vh");
        }
    },
            ...
            if (num == 2 || num == 3 || num == 4 || num == 5) {
                ...
                if (!is_bull) {
                   setTimeout(function() {
                        ...
                        $(".cardOver 0.card" + num + "41").animate({
                            left: "31.2vw"
                        }, 400);
                        $(".cardOver .card" + num + "51").animate({
                            left: "27vw"
        ...
        for (var i = 0; i < winnerNums.length; i++) {
            for (var j = 0; j < 8; j++) {
                if (appData.bankerPlayerNum == 1) {
                    $(".memberCoin" + winnerNums[i] + j).css("top", "80%");
                    ...
                    $(".memberCoin" + winnerNums[i] + j).css("top", "51%");
                    ...
                    $(".memberCoin" + winnerNums[i] + j).css("top", "28%");
                ...
                } else if (appData.bankerPlayerNum == 13) {
                    $(".memberCoin" + winnerNums[i] + j).css("top", "62%");
                    $(".memberCoin" + winnerNums[i] + j).css("left", coinLeft13);
                }
            }
        }
        //显示输家金币
