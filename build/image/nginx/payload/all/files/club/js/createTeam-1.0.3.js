...
var httpModule = {
    createTeam: function (teamName, dealerNum) {
        var data = {"name": teamName};

        Vue.http.post(globalData.apiUrl + 'club/create', data).then(function (response) {
            ...
            viewMethods.clickShowAlert(7, "创建失败");
        });
    },
};
...
var appData = {
    dealers: globalData.dealers,
    dealerName: '',
    dealer: {"dealerNum": "", "dealerName": "", "dealerCard": "0"},
    needCard: globalData.needCard,
    'createStep': 1,
...
var methods = {
    ...
    finishCreateTeam: function () {
        window.href = globalData.mhUrl;
    },
    nextStep: function () {
        if (appData.sTeamName.length < 1) {
            viewMethods.clickShowAlert(7, "请输入公会名称");
            return;
        }


        httpModule.createTeam(appData.sTeamName);
    ...
    showDealer: function () {
        if (appData.dealers.length < 1) {
            viewMethods.clickShowAlert(7, "暂无符合选区");
            return;
        }

        appData.showDealers = !appData.showDealers;
    },
    hideDealer: function () {
        console.log('hello');
        appData.showDealers = false;
    },
    selectDealerNum: function (dealer) {
        appData.showDealers = false;
        appData.dealer = dealer;
    },
};
