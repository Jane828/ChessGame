...
class Config{
	...
	const DB_Charset		= DB_CHARSET;//"utf8";

	const Processor_Port = PROCESSOR_PORT;//"12002";
	const Processor_Address =  PROCESSOR_ADDR;//"127.0.0.1";
	const Timer_Port =  TIMER_PORT;//"20099";
	const Timer_Address =  TIMER_ADDR;//"127.0.0.1";

	const GameClient_Port =  GAME_CLIENT_PORT;//"20092";
	const GameServer_Port =  GAME_SERVER_PORT;//"20091";
	const GameWeb_Port    =  GAME_WEB_PORT;//"20090";
	
	const Ticket_Mode	= TICKET_MODE;//2;	//房卡支付形式，1AA，2房主
	const Room_IsReuse	= ROOM_IS_REUSE;//0;	//房间是否重复使用，0否1是
	const GameNum_EachRound = GAME_NUM_REACH_ROUND;//6;	//每轮局数
	const Room_AutoBreak	= ROOM_AUTO_BREAK;//1;	//房间是否自动解散，0否1是
    const Log_Dir	=	 LOG_DIR2;//LOG_DIR . "lbull/";
	...
	const Rule_ScoreType_4		= 10;
	const Rule_ScoreType_5		= 20;
	const Rule_ScoreType_6      = 2;
    //用于自定义底分
    const Rule_ScoreType_7_Min  = 1;
    const Rule_ScoreType_7_Max  = 100;

	/*	上庄分数	*/
	const BankerScore_1		= 0;
	const BankerScore_2		= 300;
	const BankerScore_3		= 500;
	const BankerScore_4		= 1000;
