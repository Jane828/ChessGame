...
class Config{
	...
	const Timer_Port =  TIMER_PORT;//"30029";
	const Timer_Address =  TIMER_ADDR;//"127.0.0.1";

	const GameClient_Port =  GAME_CLIENT_PORT;//"30022";
	const GameServer_Port =  GAME_SERVER_PORT;//"30021";
	const GameWeb_Port    =  GAME_WEB_PORT;//"30020";
	
	const Ticket_Mode	= TICKET_MODE;//2;	//房卡支付形式，1AA，2房主
	const Room_IsReuse	= ROOM_IS_REUSE;//0;	//房间是否重复使用，0否1是
	const GameNum_EachRound = GAME_NUM_REACH_ROUND;//5;	//每轮局数
	const Room_AutoBreak	= ROOM_AUTO_BREAK;//1;	//房间是否自动解散，0否1是
    const Log_Dir	=	 LOG_DIR2;//LOG_DIR . "sg9/";
