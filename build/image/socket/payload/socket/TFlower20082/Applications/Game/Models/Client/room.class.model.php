    ...
    */
    public function createRoom($arrData) {
        ...
        if (!isset($data['compareProgress'])) {
            $this->logMessage('error', "function(createRoom):lack of compareProgress" . " in file" . __FILE__ . " on Line " . __LINE__);
            return $this->_missingPrameterArr($operation, "compareProgress");
        }
        if (!isset($data['seenProgress'])) {
            $this->logMessage('error', "function(createRoom):lack of seenProgress" . " in file" . __FILE__ . " on Line " . __LINE__);
            return $this->_missingPrameterArr($operation, "seenProgress");
        }
        if (!isset($data['raceCard'])) {
            $this->logMessage('error', "function(createRoom):lack of raceCard" . " in file" . __FILE__ . " on Line " . __LINE__);
        ...
        if (!isset($data['upper_limit']) || !(0 <= $data['upper_limit'] && $data['upper_limit'] <= 2000)) {
        ...
        if (!isset($data['extraRewards']) || !in_array($data['extraRewards'], [0, 5, 10, 20, 40])) {
            $this->logMessage('error', "function(createRoom):lack of extraRewards" . " in file" . __FILE__ . " on Line " . __LINE__);
        ...
        $total_num          = Config::GameNum_EachRound * $spend_ticket_count / 2;
    ...
    */
    public function joinRoom($arrData) {
        ...
        */
        ...
        $room_ary['ticket_count']    = isset($setting[Redis_Const::Room_Field_TicketCount]) ? $setting[Redis_Const::Room_Field_TicketCount] : 2;
