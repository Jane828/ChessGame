...
class Room_Model extends Public_Model
{
	...
	*/
	public function createRoom($arrData)
	{
		...
		//庄家类型
        $banker_mode = Game::BankerMode_SeenGrab;
        ...
        if ($clubPlayer['player_bean'] < $min_bean){
            $result['alert_type'] = 1;	//1房卡不足
            return array("result"=>"1","operation"=>$operation,"data"=>$result,"result_message"=>"你的欢乐豆不足");
        }
	...
	*/
	public function joinRoom($arrData)
	{
        ...
        $room_ary['is_enough'] = 1;
        $room_ary['is_member'] = 1;
		...
		*/
		...
		$room_ary['account_score'] = $clubPlayer['player_bean']; // 页面显示当前余豆
	...
	*/
	public function pullRoomInfo($arrData)
	{
		...
		if (!empty($club_id)) {
            ...
            if (!empty($clubPlayer)) {
                $playerBean = $clubPlayer['player_bean'];
            }
        }
		...
		//$game_num = $this->queryGameNumber($room_id);
		//$room_status = $this->queryRoomStatus($room_id);
		$room_ary['account_score'] = $playerBean; // 页面显示当前余豆
		$room_ary['account_status'] = $account_status;
	...
	*/
	protected function getGamerInfo($room_id,$account_id=-1)
	{
		...
		$club_id = $MMYSQL->select('club_id')->from(Room)->where('room_id='.$room_id)->single();
		if (empty($club_id)) {
		    $this->logMessage('error', "function(getGamerInfo):room($room_id) lack club_id. in file".__FILE__.' on line'.__LINE__);
		    return false;
        }
...
//                $info['account_score'] = $rScore_score;
                ...
                $clubPlayer = $MMYSQL->select('player_bean')->from(Club_Players)->where('club_id='.$club_id.' AND player_id='.$gamer_id)->row();
                $info['account_score'] = empty($clubPlayer) ? 0 : $clubPlayer['player_bean'];
	...
	*/
	public function prepareJoinRoom($arrData)
	{
        ...
        $room_ary['bean_type'] = $room_rule['bean_type'];
