    ...
    public static function dispatcherOpt($req_ary) {
        ...
        if (isset($req_ary['operation'])) {
            switch ($req_ary['operation']) {
                ...
                case 'JoinGroupNotify':
                    $Server_Timer_Model = new Server_Timer_Model();
                    $result             = $Server_Timer_Model->joinGroupNotify($req_ary);
                    break;
                case 'deleteMemberNotify':
                    $Server_Timer_Model = new Server_Timer_Model();
                    $result             = $Server_Timer_Model->deleteMemberNotify($req_ary);
                    break;
                case 'addMemberNotify':
                    $Server_Timer_Model = new Server_Timer_Model();
                    $result             = $Server_Timer_Model->addMemberNotify($req_ary);
                    break;
