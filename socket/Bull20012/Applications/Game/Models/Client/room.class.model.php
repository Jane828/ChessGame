...
class Room_Model extends Public_Model {
    ...
    */
	...
	if ($manager_id != $account_id) {
        ...
        if(!is_array($result)){
            ...
            //记录包厢当前正在进行的房间
            $BoxCurrentRoom_Key = strtr(Redis_CONST::BoxCurrentRoom_Key, $replyArr);
            $rset = $Redis_Model->saddSet($BoxCurrentRoom_Key, array("$newroom_id"));
            if($rset != OPT_CONST::SUCCESS){
                $this->logMessage('error', "function(joinbox):set box_current_room fail:key:[$BoxCurrentRoom_Key],room_id:[$newroom_id];" . ' in file ' . __FILE__ . ' on Line ' . __LINE__);
            }
        }
	...
	*/
    public function createRoom($arrData, $bflag=G_CONST::IS_FALSE) {
        ...
        $rule_type          = 1;                                 //默认分数规则
        $is_custom_score    = 0;                                 //默认自定义底分
        $has_ghost          = 0;                                 //默认不开启癞子
        $cardfour_multiple  = 1;    //四花牛
        $cardfive_multiple  = 1;    //五花牛
        $cardbomb_multiple  = 1;    //炸弹牛
        $cardtiny_multiple  = 1;    //五小牛

        $cardstraight_multiple      = 1;    //顺子牛
        $cardflush_multiple         = 1;    //同花牛
        $cardhulu_multiple          = 1;    //葫芦牛
        $cardstraightflush_multiple = 1;    //同花顺

        $banker_mode      = Game::BankerMode_FreeGrab;    //默认自由抢庄
        $banker_scoreType = Config::BankerScore_1;        //默认无上限
        $banker_score     = Config::BankerScore_1;

        $countDown_readdy = Game::LimitTime_Ready;      //准备倒计时
        $countDown_bet    = Game::LimitTime_Betting;    //下注倒计时
        $countDown_grab   = Game::LimitTime_Grab;       //抢庄倒计时
        $countDown_show   = Game::LimitTime_Show;       //摊牌倒计时
        ...
        if (isset($data['times_type']) && in_array($data['times_type'], $timesTypes)) {
            $times_type = $data['times_type'];
        } else {
            $this->logMessage('error', "function(createRoom):用户" . $account_id . " times_type error：" . " in file" . __FILE__ . " on Line " . __LINE__);
            return array("result" => "-1", "operation" => $operation, "data" => $result, "result_message" => "倍数规则错误");
        }
        ...
        } else if (isset($data['score_type']) && $data['score_type'] == 7) {
            $custom_status = isset($data['score_value']) && $data['score_value'] >= Config::Rule_ScoreType_7_Min && $data['score_value'] <= Config::Rule_ScoreType_7_Max;
            if ($custom_status) {
                $base_score      = $data['score_value'];
                $is_custom_score = 1;
            } else {
                return array("result" => "-1", "operation" => $operation, "data" => $result, "result_message" => "自定义分数规则错误");
            }
        } else {
        ...
        //分数规则
        if (isset($data['rule_type']) && $data['rule_type'] == 1) {
        ...
        } else if (isset($data['rule_type']) && $data['rule_type'] == 2) {
            $rule_type       = $data['rule_type'];
            $card7_multiple  = Game::Rule_Card7_Multiple_2;
            $card8_multiple  = Game::Rule_Card8_Multiple_2;
            $card9_multiple  = Game::Rule_Card9_Multiple_2;
            $card10_multiple = Game::Rule_Card10_Multiple_2;
        } else {
            $this->logMessage('error', "function(createRoom):用户" . $account_id . " rule_type error：" . " in file" . __FILE__ . " on Line " . __LINE__);
            return array("result" => "-1", "operation" => $operation, "data" => $result, "result_message" => "规则错误");
        }
        //特殊牌型
        if (isset($data['is_cardfour']) && $data['is_cardfour'] == 1) {
            $cardfour_multiple = Game::Rule_CardFour_Multiple;
        }

        if (isset($data['is_cardfive']) && $data['is_cardfive'] == 1) {
            $cardfive_multiple = Game::Rule_CardFive_Multiple;
        }
        if (isset($data['is_cardbomb']) && $data['is_cardbomb'] == 1) {
            $cardbomb_multiple = Game::Rule_CardBomb_Multiple;
        }
        if (isset($data['is_cardtiny']) && $data['is_cardtiny'] == 1) {
            $cardtiny_multiple = Game::Rule_CardTiny_Multiple;
        }

        //癞子牛牛新增牌型
        if (isset($data['is_straight']) && $data['is_straight'] == 1) {
            $cardstraight_multiple = Game::Rule_CardStraight_Multiple;
        }
        if (isset($data['is_flush']) && $data['is_flush'] == 1) {
            $cardflush_multiple = Game::Rule_CardFlush_Multiple;
        }
        if (isset($data['is_hulu']) && $data['is_hulu'] == 1) {
            $cardhulu_multiple = Game::Rule_CardHulu_Multiple;
        }
        if (isset($data['is_straightflush']) && $data['is_straightflush'] == 1) {
            $cardstraightflush_multiple = Game::Rule_CardStraightflush_Multiple;
        }
        //增加开启癞子
        if (isset($data['has_ghost']) && $data['has_ghost'] == 1) {
            $has_ghost = 1;
        }
        ...
        //倒计时规则
        if (isset($data["countDown"]) && count($data["countDown"]) == 4) {
            $countDown = $data["countDown"];
            if (5 <= $countDown[0] && $countDown[0] <= 10) {
                $countDown_readdy = $countDown[0];
            }
            if (5 <= $countDown[1] && $countDown[1] <= 10) {
                $countDown_grab = $countDown[1];
            }
            if (5 <= $countDown[2] && $countDown[2] <= 10) {
                $countDown_bet = $countDown[2];
            }
            if (5 <= $countDown[3] && $countDown[3] <= 10) {
                $countDown_show = $countDown[3];
            }
        }else{
            $result["alert_type"] = 1;
            return array("result" => "1", "operation" => $operation, "data" => $result, "result_message" => "倒计时设置非法");
        }
        ...
        if (!is_array($room_query) || count($room_query) == 0) {
            ...
            $r_array['player_max_num']= Config::GameUser_MaxCount;
            $r_array['total_num']    = $eachRound_totalNum;
            $r_array['times_type']   = $times_type;
        ...
        $r_mkv[Redis_Const::Room_Field_Max_Player] = Config::GameUser_MaxCount;               //游戏最大人数
        ...
        $r_mkv[Redis_Const::Room_Field_BaseScore]     = $base_score;                        //当前分数
        $r_mkv[Redis_Const::Room_Field_IsCustomScore] = $is_custom_score;                   //当前是否自定义分数
        ...
        $r_mkv[Redis_Const::Room_Field_HasGhost] = $has_ghost;                              //是否开启癞子
        $r_mkv[Redis_Const::Room_Field_Card7]    = $card7_multiple;                         //牛7倍率
        $r_mkv[Redis_Const::Room_Field_Card8]    = $card8_multiple;                         //牛8倍率
        $r_mkv[Redis_Const::Room_Field_Card9]    = $card9_multiple;                         //牛9倍率
        $r_mkv[Redis_Const::Room_Field_Card10]   = $card10_multiple;                        //牛10倍率
        $r_mkv[Redis_Const::Room_Field_CardFour] = $cardfour_multiple;                      //四花牛倍率
        $r_mkv[Redis_Const::Room_Field_CardFive] = $cardfive_multiple;                      //五花牛倍率
        $r_mkv[Redis_Const::Room_Field_CardBomb] = $cardbomb_multiple;                      //炸弹牛倍率
        $r_mkv[Redis_Const::Room_Field_CardTiny] = $cardtiny_multiple;                      //五小牛倍率

        $r_mkv[Redis_Const::Room_Field_CardStraight]      = $cardstraight_multiple;         //顺子牛倍率
        $r_mkv[Redis_Const::Room_Field_CardFlush]         = $cardflush_multiple;            //同花牛倍率
        $r_mkv[Redis_Const::Room_Field_CardHulu]          = $cardhulu_multiple;             //葫芦牛倍率
        $r_mkv[Redis_Const::Room_Field_CardStraightflush] = $cardstraightflush_multiple;    //同花顺倍率
        ...
        $r_mkv[Redis_Const::Room_Field_TimesType]       = $times_type;                      //倍数规则

        $r_mkv[Redis_Const::Room_Field_CountDown_Ready] = $countDown_readdy;                //准备倒计时规则
        $r_mkv[Redis_Const::Room_Field_CountDown_Bet]   = $countDown_bet;                   //下注倒计时规则
        $r_mkv[Redis_Const::Room_Field_CountDown_Grab]  = $countDown_grab;                  //抢庄倒计时规则
        $r_mkv[Redis_Const::Room_Field_CountDown_Show]  = $countDown_show;                  //摊牌倒计时规则
    ...
    */
    public function activateRoom($arrData) {
        ...
        if (isset($data['rule_type']) && $data['rule_type'] == 1) {
            ...
            $this->logMessage('error', "function(activateRoom):用户" . $account_id . " rule_type error：" . " in file" . __FILE__ . " on Line " . __LINE__);
            return array("result" => "1", "operation" => $operation, "data" => $result, "result_message" => "规则错误");
        }
        //特殊牌型
        if (isset($data['is_cardfour']) && $data['is_cardfour'] == 1) {
    ...
    */
    public function joinRoom($arrData) {
        ...
        } else    //未加入
        {
            ...
            if($box_number && $box_number != Game::ORDINARY_ROOM){
                //判断是否有空位，更新包厢空房记录
                if(($user_count + 1) >= Config::GameUser_MaxCount){
                    $replyArr["[boxnumber]"] = $box_number;
                    $BoxEmptyRoom_Key = strtr(Redis_CONST::BoxEmptyRoom_Key, $replyArr);
   ...
   */
    public function audienceWatch($arrData) {
        ...
        $room_ary['room_id']     = $room_id;
        $room_ary['room_number'] = $room_number;
        $room_ary['room_status'] = $room_data['stat'];
        $room_ary['game_num']    = $room_data['gnum'];
        $room_ary['total_num']   = $room_data['totalnum'];
        $room_ary['serial_num']  = $serial_num;
    ...
    */
    public function pullRoomInfo($arrData) {
        ...
        //$pull_return = '{"result":0,"operation":"PullRoomInfo","data":{"total_num":"24","base_score":"5","account_score":"0","account_status":"3","ticket_checked":1,"cards":["B8","B2","A13","D3","-1"],"card_type":0,"combo_array":[],"combo_point":0},"result_message":"拉取房间信息","all_gamer_info":[{"serial_num":"1","account_id":117,"nickname":"態度","headimgurl":"http://wx.qlogo.cn/mmhead/Q3auHgzwzM6zicpFsEC72C582whyhzH1gQiaHdtmND81stkduDPG06Pw/0","account_score":"0","account_status":"3","online_status":1,"ticket_checked":1,"is_banker":0,"cards":[],"card_type":0,"combo_array":[],"combo_point":0,"multiples":false,"banker_multiples":"1"},{"serial_num":"2","account_id":140,"nickname":"西京医院","headimgurl":"http://wx.qlogo.cn/mmhead/PiajxSqBRaELT9ow6ayxofickIj4ibVlwRpJTyhUAPbsnAVibaZ7zdAKEw/0","account_score":"0","account_status":"3","online_status":1,"ticket_checked":1,"is_banker":0,"cards":[],"card_type":0,"combo_array":[],"combo_point":0,"multiples":false,"banker_multiples":"1"},{"serial_num":"3","account_id":113,"nickname":"魔镜","headimgurl":"http://wx.qlogo.cn/mmhead/Q3auHgzwzM4icAiahlq0SQ0RyQfIYOGFyYa3aH9MkamSLd7ubmYOCz7w/0","account_score":"0","account_status":"0","online_status":1,"ticket_checked":0,"is_banker":0,"cards":[],"card_type":0,"combo_array":[],"combo_point":0,"multiples":false,"banker_multiples":"1"},{"serial_num":"4","account_id":369,"nickname":"梧桐树","headimgurl":"http://wx.qlogo.cn/mmhead/PiajxSqBRaEKVgSuBW7diaib8a8SCm7KPXtQDEvla9ibFr4s2gfaDPAbmg/0","account_score":"0","account_status":"3","online_status":1,"ticket_checked":1,"is_banker":0,"cards":[],"card_type":0,"combo_array":[],"combo_point":0,"multiples":false,"banker_multiples":"1"},{"serial_num":"6","account_id":109,"nickname":"勿忘我勿怪我！","headimgurl":"http://wx.qlogo.cn/mmhead/Q3auHgzwzM4veNL95b40f29ICn0KHpIO72a4bhpKZ6jIExj7VwQ6Vw/0","account_score":"0","account_status":"3","online_status":1,"ticket_checked":1,"is_banker":0,"cards":[],"card_type":0,"combo_array":[],"combo_point":0,"multiples":false,"banker_multiples":"1"}]}';
    ...
    */
    public function prepareJoinRoom($arrData) {
        ...
        $room_ary['base_score']  = $room_rule['baseScore'];
        $room_ary['rule_type']   = $room_rule['rule_type'];
        $room_ary['times_type']  = $room_rule['times_type'];
        $room_ary['is_cardfour'] = $room_rule['is_cardfour'];
        $room_ary['is_cardfive'] = $room_rule['is_cardfive'];
        $room_ary['is_cardbomb'] = $room_rule['is_cardbomb'];
        $room_ary['is_cardtiny'] = $room_rule['is_cardtiny'];

        $room_ary['is_straight']      = $room_rule['is_straight'];
        $room_ary['is_flush']         = $room_rule['is_flush'];
        $room_ary['is_hulu']          = $room_rule['is_hulu'];
        $room_ary['is_straightflush'] = $room_rule['is_straightflush'];
        $room_ary['has_ghost']        = $room_rule['hasGhost'];
