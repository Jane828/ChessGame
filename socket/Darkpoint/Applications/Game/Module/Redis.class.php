...
class Redis_Model extends Base_Model
{
	...
	*/
	...
	public function zremSetbyscore($set = G_CONST::EMPTY_STRING,$min = G_CONST::EMPTY_STRING,$max = G_CONST::EMPTY_STRING)
	{
		...
		var_dump("----zremSetbyscore------$set, $min, $max--------");
    ...
    */
    public function harraySet($key,$value)
    {
        $redisAuth = $this->pingRedisAuth();
        ...
        if($key === G_CONST::EMPTY_STRING)
        {
            $this->logMessage('error', "function(arraySet):$key is empty string"." in file".__FILE__." on Line ".__LINE__);
            return Redis_CONST::DATA_NONEXISTENT;
        }
        if(!is_array($value))
        {
            $this->logMessage('error', "function(arraySet):$value is not an array"." in file".__FILE__." on Line ".__LINE__);
            return Redis_CONST::DATA_NONEXISTENT;
        }
        $value = serialize($value);
        $redisResult = $redisAuth->set($key,$value) ;
    ...
    */
    public function harrayGet($key)
    {
        ...
        $redisResult = $redisAuth->get($key);
        $value_serl = @unserialize($redisResult);
