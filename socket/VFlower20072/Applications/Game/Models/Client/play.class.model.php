...
class Play_Model extends Public_Model
{
    ...
    */
    public function chooseChip($arrData)
    {
        ...
        if (!isset($data['score']) || !in_array($data['score'], [2, 10, 4, 8, 16, 20, 5, 40, 80])) {
        ...
        if (empty($clubPlayer)) {
            return ['result' => OPT_CONST::FAILED, 'operation' => $operation, 'data' => $result, 'result_message' => '您不在公会中'];
        }
        /* 用限入判断下注
        ...
        if ($minBean > $clubPlayer['player_bean']) {
        */
        // 用余豆判断下注
        if ($score > $clubPlayer['player_bean']) {
            return ['result' => OPT_CONST::FAILED, 'operation' => $operation, 'data' => $result, 'result_message' => '您的欢乐豆不足'];
        }

        $this->deleteRoomTimer($room_id);

        $now_bean = $clubPlayer['player_bean'] - $score;

        $MMYSQL->update(Club_Players)->cols(['is_gaming' => 1, 'player_bean' => $now_bean, 'updated_at' => date('Y-m-d H:i:s')])
    ...
    */
    public function openCard($arrData)
    {
        ...
        if ($benchmark > $clubPlayer['player_bean']) {
            return ['result' => OPT_CONST::FAILED, 'operation' => $operation, 'data' => $result, 'result_message' => '您的欢乐豆不足'];
        }

        $now_bean = $clubPlayer['player_bean'] - $benchmark;
