...
var httpModule = {
    getActivityInfo: function() {
        var postData = {"account_id": userData.accountId, "dealer_num": globalData.dealerNum, "session": globalData.session,'room_number' : globalData.roomNumber ,'game_type' : 9 };
    ...
    getScoreInfo: function () {
        Vue.http.post(globalData.baseUrl + 'f/scoreStat', {'type':9}).then(function (response) {
            var bodyData = response.body;
...
var socketModule = {
    ...
    processGameRule: function(obj) {
        ...
        if (appData.ruleInfo.isCardfive == 1 && appData.ruleInfo.isCardbomb == 1 && appData.ruleInfo.isCardtiny === 1) {

            appData.ruleInfo.rule_height = '8vh';
...
var viewMethods = {
    ...
    resetCardOver: function(num) {
        if (num == 1) {
            ...
            $(".cardOver .card911").css("left", "10.5vh");
        ...
        for (var i = 0; i < winnerNums.length; i++) {
            for (var j = 0; j < 8; j++) {
                if (appData.bankerPlayerNum == 1) {
                    ...
                    $(".memberCoin" + winnerNums[i] + j).css("top", "43%");
                    ...
                    $(".memberCoin" + winnerNums[i] + j).css("top", "6%");
