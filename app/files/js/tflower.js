...
var httpModule = {
    getActivityInfo: function() {
        Vue.http.post(globalData.baseUrl + 'f/getActivityInfo', { "account_id": userData.accountId, "dealer_num": globalData.dealerNum,'room_number' : globalData.roomNumber ,'game_type' : 110 }).then(function(response) {
    ...
    getScoreInfo: function () {
        Vue.http.post(globalData.baseUrl + 'f/scoreStat', {'type':110}).then(function (response) {
            var bodyData = response.body;
...
var socketModule = {
    ...
    processPrepareJoinRoom: function(obj) {
        ...
        if (obj.data.chip_type) {
            ...
            appData.ruleInfo.allow235GTPanther = obj.data.allow235GTPanther || 0;
            if (appData.ruleInfo.disable_pk_100 == 1 && appData.ruleInfo.disable_pk_men == 1) {
                appData.ruleInfo.rule_height = 60;
            } else {
                appData.ruleInfo.rule_height = 30;
        ...
        for (var i = 0; i < 10; i++) {
            ...
            for (var j = 0; j < obj.data.length; j++) {
                if (appData.player[i].account_id == obj.data[j].account_id) {
                    ...
                    appData.player[i].online_status = Math.ceil(obj.data[j].online_status);
                    appData.player[i].account_score = appData.player[i].account_score - parseInt(obj.data[j].chip);
                    ...
                    appData.game.score = appData.game.score + parseInt(obj.data[j].chip);
    ...
    throwCoin: function(num, score, tim) {
        if (tim){
            setTimeout(function () {
                ...
                $(".coin" + num).velocity({ top: (per * 250 - 28) * Math.random(), left: (per * 150 - 28) * Math.random() }, {
                    duration: 300,
                    complete: function() {
...
var gameWidth = document.body.clientWidth + 'px' ;
var gameHeight = document.body.clientHeight + 'px' ;
var initView = function initView() {

    $('#app-main').width(gameWidth);
    $('#app-main').height(gameHeight);
    $('#table').width(gameWidth);
    $('#table').height(gameHeight);
    ...
    window.onload = function() {
        ...
        var member6Top = (window.innerHeight * 0.195 - 28 - 89) / 2 + 26;
        member6Top = (member6Top / window.innerHeight) * 100;

        $('.member6').css('top', member6Top + '%');
    };
...
var methods = {
    ...
    closeBack: viewMethods.closeBack,
    showWatch: viewMethods.showWatch,
    hideWatch: viewMethods.hideWatch,
...
$(function() {
    ...
    $(".place").css("width", per * 150);
    $(".place").css("height", per * 350);
    $(".place").css("top", per * 300);
...
function getShareContent() {
    ...
    if (appData.ruleInfo.ticket_count == 2) {
        shareContent += ' 局数：10局x2张房卡';
    } else {
        shareContent += ' 局数：20局x4张房卡';
    }
