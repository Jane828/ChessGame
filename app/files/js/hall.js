...
app.controller("myCtrl", function($scope,$http,$interval) {
    ...
    $scope.downUpperLimit = function(type){
        ...
        switch (type){
            ...
            case 'b':
                ...
                return false;
            case 'dp10':
                limit = parseInt($scope.createInfo.dpoint.upper_limit);
                $scope.createInfo.dpoint.upper_limit = limit < 100? limit: limit - 100;
                return false;
            default:
    ...
    $scope.upUpperLimit = function(type){
        ...
        switch (type){
            ...
            case 'dp10':
                limit = parseInt($scope.createInfo.dpoint.upper_limit);
                $scope.createInfo.dpoint.upper_limit = limit == 2000? limit: limit + 100;
                return false;
            default:
                limit = parseInt($scope.createInfo.flower.upper_limit);
        ...
        $scope.ws.onopen = function(){
            ...
            if(type==1){
                ...
                $scope.ws.send(JSON.stringify({
                    ...
                    "data":{
                        "data_key":Date.parse(new Date())+randomString(5),
                        'chip_type': $scope.createInfo.dpoint.chip_type,
                        'ticket_count':$scope.createInfo.dpoint.ticket_count,
                        "banker_mode":$scope.createInfo.dpoint.banker_mode,
                        'upper_limit':$scope.createInfo.dpoint.upper_limit,
                        'first_lossrate':$scope.createInfo.dpoint.first_lossrate,
                        'second_lossrate':$scope.createInfo.dpoint.second_lossrate,
                        'three_lossrate':$scope.createInfo.dpoint.three_lossrate,
                        'game_type': type,
                        'countDown': $scope.createInfo.dpoint.countDown,
                    }
                }));
            }
        };
    ...
    $scope.selectChange=function(type,num){
        ...
        else if($scope.createInfo.isShow==61){
            if(type==1){
                var arrObj = {1:[10,20,30,50,100]};
                $scope.createInfo.dpoint.chip_type = arrObj[num];
            }
            else if(type==3){
                $scope.createInfo.dpoint.ticket_count=num;
            }
            else if(type==4){
                $scope.createInfo.dpoint.upper_limit=num;
            }
            else if(type==5){
                $scope.createInfo.dpoint.first_lossrate=num;
            }
            else if(type==6){
                $scope.createInfo.dpoint.second_lossrate=num;
            }
            else if(type == 7){
                $scope.createInfo.dpoint.three_lossrate=num;
            }
        }
    };
    ...
    $scope.createDpoint=function($event){
        $event.stopPropagation();
        $(".createRoom .mainPart").css({
            'height':'80vh',
            'padding': '5px 5px',
    ...
    $scope.selectBankerDpoint = function (type) {
        if (type == 1) {
            $scope.createInfo.dpoint.score_type = 1;
            $scope.createInfo.dpoint.banker1 = "selected";
            $scope.createInfo.dpoint.banker2 = "unselected";
            $scope.createInfo.dpoint.banker3 = "unselected";
        } else if (type == 2) {
            $scope.createInfo.dpoint.score_type = 1;
            $scope.createInfo.dpoint.banker1 = "unselected";
            $scope.createInfo.dpoint.banker2 = "selected";
            $scope.createInfo.dpoint.banker3 = "unselected";
        } else if (type == 3) {
            $scope.createInfo.dpoint.score_type = 1;
            $scope.createInfo.dpoint.banker1 = "unselected";
            $scope.createInfo.dpoint.banker2 = "unselected";
            $scope.createInfo.dpoint.banker3 = "selected";
        } 
        $scope.createInfo.dpoint.banker_mode = type;
    };
    $scope.createCommit = function () {
        if ($scope.userInfo.card>0){
            ...
            if($scope.createInfo.isShow==1){
                ...
                $scope.connectSocket($scope.socket.dpoint,61);
            }
...
function getSetting(default_setting) {
    ...
    if(createInfo){
        ...
        if(typeof(createInfo.dpoint) !== "undefined"){
            cloneObj(default_setting.dpoint, createInfo.dpoint);
            if((typeof default_setting.dpoint.chip_type).toLowerCase() != 'object' || default_setting.dpoint.chip_type.indexOf(0) !== -1){
                default_setting.dpoint.chip_type = [];
            }
        }
