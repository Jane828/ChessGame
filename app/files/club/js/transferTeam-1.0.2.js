...
var httpModule = {
    getEarnings: function () {
        ...
        var data = {"account_id": userData.accountId, "dealer_num":globalData.dealerNum, "page": appData.page, "aid":userData.aid, 
            "s":userData.s};

        Vue.http.post(globalData.baseUrl + 'activity/getPhoneAccountList', data).then(function(response) {
            logMessage(response.body);
            var bodyData = response.body;
            if (bodyData.result == 0) {
                var resultData = bodyData.data;
                appData.totalEarnings = bodyData.balance_amount;
                appData.page = bodyData.page;
                appData.sumPage = bodyData.sum_page;

                for (var i = 0; i < resultData.length; i++) {
                    var item = resultData[i];
                    appData.members.push({"name":item.nickname, "avatar":item.headimgurl,"account_id":item.account_id,"account_code":item.account_code,"ticket":item.ticket_count,"is_cantransfer":item.is_cantransfer,"checked":false});
                }
    ...
    confirmTranfer: function (withdraw_id) {
        var data = {"account_id": userData.accountId, 
        "dealer_num":globalData.dealerNum, 
        "sourceAccountId": appData.selectedItem.account_id, 
        "targetAccountId": appData.receiveItem.account_id, 
        "aid":userData.aid, 
        "s":userData.s};

        Vue.http.post(globalData.apiUrl + '/org/transfer', data).then(function(response) {

        	appData.isHttpRequest = 0;

            viewMethods.clickCloseAlert();
            
            var bodyData = response.body;
            var card1 = appData.selectedItem.ticket;
            var card2 = appData.receiveItem.ticket;

            if (bodyData.result == 0) {
                appData.showTranfer = false;
                for (var i = 0; i < appData.members.length;i++) {
                    var temp1 = appData.members[i];
                    if (temp1.account_id == appData.receiveItem.account_id) {
                        temp1.ticket = parseInt(card1) + parseInt(card2);
                        temp1.is_cantransfer = 1;
                        console.log(temp1);
                    } else if (temp1.account_id == appData.selectedItem.account_id) {
                        temp1.ticket = 0;
                        temp1.is_cantransfer = 0;
                    }
                }
                ...
                methods.showAlertInfo(1,bodyData.msg);
...
if (globalData.phoneUsers.length >= 1) {
	for (var i = 0; i < globalData.phoneUsers.length; i++) {
		var item = globalData.phoneUsers[i];
		appData.members.push({"name":Base64.decode(item.name), "avatar":item.head_img_url,"account_id":item.account_id,"account_code":item.account_code,"ticket":item.ticket_count,"is_cantransfer":item.is_cantransfer,"checked":false});
	}
}
...
var methods = {
    ...
    clickTranfer:function () {
        if (appData.receiveItem == null || appData.receiveItem == undefined) {
            viewMethods.clickShowAlert(7,'请先选择转移账号');
            return;
        }
    ...
    confirmTranfer: function () {
        if (appData.receiveItem == null || appData.receiveItem == undefined) {
            viewMethods.clickShowAlert(7,'请先选择转移到哪个账号');
            return;
        }

        if (appData.isHttpRequest == 1) {
        	return;
        }
    ...
    clickShowTranfer:function (item) {
        appData.tMembers = [];
        for (var i = 0; i < appData.members.length; i++) {
            var temp = appData.members[i];
            if (temp.account_id != item.account_id) {
                temp.checked = false;
                appData.tMembers.push(temp);
            }
        }

        appData.showTranfer = true;
    ...
    clickTMember:function (item) {
        for (var i = 0; i < appData.tMembers.length; i++) {
            var temp = appData.tMembers[i];
            temp.checked = false;
        }

        appData.receiveItem = item;
        appData.receiveID = item.account_id;
        item.checked = true;
    },
    showAlertInfo:function(type,text){
...
function refreshTBScroll() {
	// Vue.nextTick(function () {
	// 	if (!appData.tBScroll) {
	// 		appData.tBScroll = new BScroll(document.getElementById('tMemberDiv'), {
	// 			startX: 0,
	// 			startY: 0,
	// 			scrollY: true,
	// 			scrollX: false,
