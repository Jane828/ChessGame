...
class Db_Models extends CI_Model
{
	...
	log_message("info", "liuyongnew==========".$sql);
	log_message("info", "liuyongnew==========".$query->num_rows());
	 ...
	 * @return int
	 */
	public function deleteFunc($database, $tableName, $where){
		$db_obj = $this->load->database($database, TRUE);
		$db_obj->delete($tableName, $where);
		$affNum = $db_obj->affected_rows();
		if($affNum > 0){
			$result = $affNum;
		} else {
