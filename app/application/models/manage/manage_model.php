...
class Manage_Model extends Public_Models {
    ...
    public function joinGroup($arrData) {
        ...
        if (DB_CONST::DATA_NONEXISTENT != $join_query) {
            if (1 == $join_query['status'] && $join_query['is_delete'] == 0) {
                ...
                $result['status'] = 3;
                return array('result' => OPT_CONST::FAILED, 'data' => $result, 'result_message' => "对方禁止加为好友");
            } else {
                ...
                $data = array("account_id"=> $manager_id);
    ...
    */
    public function searchMember($arrData) {
        ...
        //查找字母首字符开头的好友
        $sql        = 'select ' . WX_Account . '.user_code,nickname,headimgurl,capital,' . Manage_Member . '.create_time,member_id,status,aliases,attention  from ' . WX_Account . ',' . Manage_Member . " where  $where  and capital >= 'A' order by `capital` limit  $offset,$limit";
        ...
        if ($list_query != DB_CONST::DATA_NONEXISTENT) {
            ...
            foreach ($list_query as $list_item) {
                ...
                $array['attention']              = $list_item['attention'];
        ...
        if ($limit2 > 0) {
            ...
            $sql         = 'select ' . WX_Account . '.user_code,nickname,headimgurl,' . Manage_Member . '.create_time,member_id,status, aliases, attention  from ' . WX_Account . ',' . Manage_Member . ' where ' . $where . ' and capital < "A" order by `capital`' . ' desc limit ' . $offset . ',' . $limit2;
    ...
    */
    public function dealMember($arrData) {
        ...
        if (DB_CONST::DATA_NONEXISTENT != $join_query) {
            ...
            if ($type == G_CONST::MEMBER_RELATION_REFUSE) {
                $updateMember_str .= ",is_delete=1";
            }

            $updateMember_where = 'member_id=' . $member_id;

            if ($join_query["status"] != G_CONST::MEMBER_RELATION_BLACK && $type == G_CONST::MEMBER_RELATION_FRIND) {
            ...
            $updateMember_query = $this->changeNodeValue($dealerDB, Manage_Member, $updateMember_str, $updateMember_where);
            if ($join_query["status"] != G_CONST::MEMBER_RELATION_BLACK && $type == G_CONST::MEMBER_RELATION_FRIND) {
                $user_code = $manager_id + G_CONST::USRECODE_ACCOUNTID_SUB;
                $data = array("account_id"=> $join_query["account_id"], "user_code"=> $user_code);
                $this->sendToFrontWebsocket("addMemberNotify", $data);
            }
            $result['status']   = $type;
    ...
    public function deleteMember($arrData) {
        ...
        if (DB_CONST::DATA_NONEXISTENT != $join_query) {
            ...
            if ($join_query["status"] != G_CONST::MEMBER_RELATION_NONE){
                $user_code = $manager_id + G_CONST::USRECODE_ACCOUNTID_SUB;
                $data = array("account_id"=> $join_query["account_id"], "user_code"=> $user_code);
                $this->sendToFrontWebsocket("deleteMemberNotify", $data);
            }
    ...
    public function setManagerUser($arrData) {
        ...
        $update_str   = "attention = if(attention=1, 0, 1)";
        $update_where = "member_id = " . $member_id . " and is_delete = 0";
    ...
    public function getRoomList($arrData) {
        ...
        $account_id = $arrData['account_id'];
        $limit      = 10;
        $friend_status = G_CONST::MEMBER_RELATION_FRIND;
        ...
        switch ($arrData['game_category']) {
            ...
            case "bull":
                ...
                break;
	    case "anbao":
                $room_db   = $this->config->item("anbao", "game")["table"];
                $game_type = array_reduce(array("anbao"), $gettype, "anbao");
                break;
            default:
                return array('result' => OPT_CONST::FAILED, 'data' => $result, 'result_message' => "请选择正确的游戏类型");
        }

        $manager_sql   = "select account_id from " . Manage_Member . " where account_id in (select manager_id from ". Manage_Member ." where account_id= $account_id and status = $friend_status) and manager_id =  $account_id  and attention = 1 and is_delete = 0 and status= $friend_status";
        $manager_query = $this->getDataBySql($dealerDB, 0, $manager_sql);

        $mid_arr = array();
        foreach ($manager_query as $manage) {
            $mid_arr[] = $manage["account_id"];
        }
        $mid_arr[] = $account_id;
        ...
        if (count($result) < $limit) {
            ...
            if (DB_CONST::DATA_NONEXISTENT != $room_query) {
                ...
                foreach ($room_query as $room_item) {
                    ...
                    $tmp_data["user_count"] = $redis->zard($prefix . "RoomSeq:" . $room_item["room_id"]);
                    $result[] = $tmp_data;
                }
            }
        }
    ...
    public function GetRoomInfo($arrData) {
        ...
        $room_config = json_decode($query["room_config"], true);
        if (is_array($room_config)){
            $room_config["game_type"] = + $game_type;
        }
        $result["config"] = json_encode($room_config);
