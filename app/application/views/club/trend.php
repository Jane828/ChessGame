...
<title>公会动态</title>
<link rel="shortcut icon" type="image/x-icon" href="http://fy.one168.com/files/club/images/card/logo.png" />
...
<body  >
	<div id="app-main"  v-cloak>
		...
		<div  class="item" v-for="b in beanList">
			<div class="dot yellow1"></div>
			<div class="time">{{b.time}}</div>
			<div class="inner">
                <a v-if="b.type==1">会长创建公会</a>
                <a v-if="b.type==2">会长将公会改名为</a>

				<a class="bule2" v-if="b.type==3||b.type==4||b.type==5">{{b.nick}}</a>
				<a v-if="b.type==3">加入公会</a>
				<a v-if="b.type==4">退出公会</a>
				<a v-if="b.type==5">被会长踢出公会</a>
				
				<a class="yellow2" v-if="b.type==1||b.type==2">{{b.name}}</a>

				<a class="restBean" v-if="b.type==3||b.type==4||b.type==5">,余额{{b.bean}}豆.</a>
			</div>
			
		</div>	
	
		<div class="addMore" v-if="page!=totalPage&&totalPage!=0" v-on:click="dynamics">点击加载更多</div>
...
var methods={
	dynamics:function(){
		appData.page+=1;
		Vue.http.get(globalData.apiUrl+'club/dynamics?club_no='+appData.club_no+"&page="+appData.page).then(function(res){
